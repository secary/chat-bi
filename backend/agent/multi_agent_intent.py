...
def classify_multi_agent_intent(messages: List[Dict[str, str]]) -> Optional[Dict[str, Any]]:
    ...
    wants_decision = _contains_any(user_text, _DECISION_KEYWORDS)
...
def build_initial_plan_from_intent(intent: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    routes = intent.get("required_routes")
    if not isinstance(routes, list) or not routes:
        return None
    first_route = str(routes[0] or "").strip()
    if not first_route:
        return None
    return {
...
def build_next_plan_from_intent(
    ...
    *,
    ...
    routes = [str(item) for item in intent.get("required_routes", []) if str(item).strip()]
    for route in routes:
