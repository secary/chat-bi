...
def _parse_legacy_frontmatter(block: str) -> dict[str, Any]:
...
def parse_frontmatter(text: str) -> tuple[dict[str, Any], str]:
    ...
    try:
        ...
        pass
    return _parse_legacy_frontmatter(block), body


def _skill_doc_from_path(path: Path, meta: dict[str, Any], body: str) -> SkillDoc:
    return SkillDoc(
...
def scan_skills(skills_dir: Path) -> List[SkillDoc]:
    ...
    for path_str in glob(pattern):
        ...
        docs.append(_skill_doc_from_path(path, meta, body))
    return docs
...
## 约束
- 以完成用户的查询意图为目标，不应机械执行"调用工具→结束"流程。
- 收到 Observation 后必须基于其中的数值与事实作答，禁止编造数据。
- 若只需一次 Skill：调用 `call_skill`，然后 `finish`。
- 若用户意图不清晰或缺少关键信息（如字段名、时间范围、筛选条件等）：输出 `ask` 主动询问补充细节，不得猜测或臆断。
- 若上一个 Skill 返回的 Observation 中只有 `preview_rows`（预览行，默认5条）而没有完整 `rows`，说明数据不完整。必须继续调用 `chatbi-chart-recommendation` 或在 skill_args 中传入 `--include-rows` 以获取完整行后再分析，不得在数据不完整时输出 `finish`。
- 每轮最多安排一次 `call_skill` 或 `ask`；需要多个 Skill 时请分多轮输出。
...
def _append_bullets(parts: List[str], title: str, items: Sequence[str]) -> None:
    if not items:
        return
    parts.append(title)
    for item in items:
...
def _skills_markdown_lines(skills_docs: Sequence[SkillDoc]) -> List[str]:
    ...
    for doc in skills_docs:
        ...
        _append_bullets(parts, "**选用时机**：", doc.trigger_conditions)
        _append_bullets(parts, "**不要用**：", doc.when_not_to_use)
        _append_bullets(parts, "**必备上下文**：", doc.required_context)
