...
- 仅当用户明确只要查询演示库业务表、且与上传文件无关时，才使用 `chatbi-semantic-query`。
- 一旦 `chatbi-file-ingestion` 已返回 Observation：
  - 若数据已完整（包含 `rows`），可直接进行后续分析或 `finish`
  - 若数据不完整（只有 `preview_rows`），应继续分析或调用 `chatbi-chart-recommendation`，不得重复调用 `chatbi-file-ingestion` 读取同一文件
  - 仅在需要刷新数据或用户明确要求时，才重新调用 `chatbi-file-ingestion`
