...
- 仅当用户明确只要查询演示库业务表、且与上传文件无关时，才使用 `chatbi-semantic-query`。
- 一旦 `chatbi-file-ingestion` 已返回足够的 Observation，同一轮不要重复调用它读取同一文件；后续应优先 `finish`，或在需要图表时调用 `chatbi-chart-recommendation`。
...
- **数据完整性检查**：若上一个 Skill 返回的 Observation 中只有 `preview_rows`（预览行，默认5条）而没有完整 `rows`，说明当前数据不完整。此时不得输出 `finish`，必须继续调用 `chatbi-chart-recommendation` 或其他分析技能来处理完整数据，或明确在 skill_args 中传入 `--include-rows` 以获取完整行后再分析。
"""
