...
from backend.agent.executor import find_skill, latest_user_content, run_script, skill_args_for_execution
...
def _build_steps(plan: Dict[str, Any], messages: List[Dict[str, str]]) -> List[Dict[str, Any]]:
    ...
    if _is_query_plus_decision(messages):
        ...
        return [
            {"skill": "chatbi-semantic-query", "skill_args": [user_text], "plan": query_plan, "phase": "查询"},
            {"skill": "chatbi-decision-advisor", "skill_args": [user_text], "plan": {}, "phase": "建议"},
        ]
    return [{"skill": plan["skill"], "skill_args": plan.get("skill_args", []), "plan": plan, "phase": "查询"}]
    ...
    for step in steps:
        ...
        try:
            ...
            if skill_name == "chatbi-decision-advisor" and previous_result:
                dim = _infer_primary_dimension(previous_result)
                if dim and args:
                    args = [f"{args[0]}，重点分析维度：{dim}"]
            ...
            log_event(trace_id, "agent.skill", "completed", payload={"skill": skill_name, "kind": result.get("kind")})
