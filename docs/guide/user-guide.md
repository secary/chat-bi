...
- **宿主机开发**：`bash scripts/start_dev.sh` 启动 MySQL 容器，并在宿主机运行 FastAPI reload 与 Vite；前端 **5173**，后端 **8000**，MySQL 宿主机 **33067**。`docker-compose.dev.yml` 中 `chatbi-db-dev` 使用 Docker named volume 保存 `chatbi_demo`。
