...
- **文件与图像**：上传 CSV/XLSX 做结构校验与预览；上传图片时可配合视觉模型抽取表格纳入上下文（取决于 LLM Profile 是否开启 Vision）。
...
- **数据仪表盘**：独立页面展示当前业务库的聚合 KPI 与固定图表。
...
| `/dashboard` | 仪表盘 | 所有登录用户 | 业务库聚合 KPI、固定图表 |
...
| `/llm` | LLM 配置 | admin | 多 Profile、激活、Vision、测试连接 |
...
- **多专线协作**：开启后走 Manager + 多专线子 Agent（见 §6.3）。状态保存在本地。
- **导出 PDF 报告**：基于当前会话生成 PDF；依赖容器内字体与 PDF 引擎。
...
- **附件**：表格类（CSV/XLSX）上传后预填含服务端路径的文案；图像类（PNG/JPG/WebP）可触发视觉抽取（需 Vision Profile）。
...
不经对话快速浏览**当前生效业务库**聚合：销售总额、订单条数、时间范围、区域数、区域占比饼图、按月柱状图等；顶部可能对库表异常给出警告。
...
查看并切换各 Skill 的启用状态（`chatbi_admin_skill_registry`）。禁用后不会进入 Agent 的「可用 Skill」列表。
...
- **Vision 开关**：标记 Profile 是否支持图像表格抽取；可与全局视觉设置配合。
...
| 上传 → 提案 → 看板 | 上传 CSV 后发送预填提示 →「采纳全部指标」 | 指标提案卡片 → Auto Analysis 看板 |
| 上传图像 | 上传表格截图 +「读取图中表格」 | 视觉抽取后结合问答（需 Vision） |
