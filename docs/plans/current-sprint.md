...
| 92 | 将查询计划实时并入思考步骤：`backend/agent/formatter.py` 在 `plan_summary` 存在时先额外发出一条 `thinking`，内容形如“查询计划：指标=…；维度=…；时间条件=…”，让现有 `ThinkingBubble` 无需额外前端 UI 即可显示；补 `test_agent_skill_protocol.py` 校验事件顺序与文案 | 当前思考步骤里展示的是紧凑调试文案，`time_filter` 仍是 SQL 条件串；若后续要更用户友好，可再补自然语言时间标签 | 本地执行 `PYTHONPATH=. .venv/bin/python scripts/run_tests.py quick -- -q`，刷新页面后新发一条问数消息，确认“思考步骤”里新增一条“查询计划：...” |
