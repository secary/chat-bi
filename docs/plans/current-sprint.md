...
| 36 | 修复思考步骤重复：`useChat` 中 SSE 事件更新从“修改 last 引用对象”改为不可变更新，避免 React StrictMode 下同一事件被双次副作用追加（`thinking`/`text`/`error`） | 该问题主要在开发模式更易复现；生产构建通常不触发 StrictMode 双调用，但仍应修复状态更新纯度 | 刷新前端后发送任意消息，确认“思考步骤”每条只出现一次 |
