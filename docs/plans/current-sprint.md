...
| 44 | 开发环境可关闭用户登录：`CHATBI_AUTH_ENABLED` / `VITE_AUTH_ENABLED`（未设置视为开启以保护生产）；`docker-compose.dev.yml` 默认关闭；免登录时会话归属 `CHATBI_AUTH_DEV_USER_ID`（默认种子管理员 id=1） | 本机全量 `pytest` 仍会因 Windows 下误收集 `database/mysql-data*`、以及过时 runner 导入报错（既有问题） | 开发容器内需前后端开关一致；本地非 Docker 开发时在 `.env`/`env.dev` 同步两处变量 |
