...
| 97 | 修复 CI Python 格式：`ruff format` 统一 `memory_repo.py`、`session_repo.py`、`user_repo.py` 中 SQL 字面量换行；`trace.py` 的 `create_trace_table_sql` 改为先 `strip()` 再单行 `replace`，避免 Black 与 Ruff formatter 对链式调用换行不一致；`observation.py` 去掉无意义的行尾 `#` | 与 `.github/workflows/ci.yml` 中 `ruff check` + `black --check` 一致；若流水线额外启用 `ruff format --check`，本改动亦对齐 | 推送前执行 `ruff check backend/ scripts/ tests/`、`black --check backend/ scripts/ tests/`，可选 `ruff format --check backend/ scripts/ tests/` |
| 98 | 修复 GitHub Actions `Backend pytest` 的 Black 检查失败：删除 `backend/agent/formatter.py`、`backend/agent/observation.py` imports 与模块说明之间的多余空行，使 `PYTHONPATH=. .venv/bin/python -m black --check backend/ scripts/ tests/` 与 CI 输出一致恢复为绿 | 本次是纯格式修复，不涉及运行时逻辑；当前环境无 `ruff` 可执行命令，但不影响定位这次 `black --check` 失败 | 本地执行 `PYTHONPATH=. .venv/bin/python scripts/run_tests.py foundation agent -- -q` 与 `PYTHONPATH=. .venv/bin/python -m black --check backend/ scripts/ tests/`，确认测试入口和格式检查都通过 |
...
| 108  | 修复 Windows 下 `test_bootstrap_dev_script.py` 的 bash 兼容问题：测试写入临时 `bootstrap_dev.sh` 和 fake `.venv/bin/python` 时显式 `newline="\n"`；fake python 改为向 repo 当前目录写日志，避免 Git Bash 解析 `TMPDIR=C:\...`；并新增 `.gitattributes` 固定 shell/hook 文件为 LF                                                                                                                                                                                          | 根因一是 CRLF 让 bash 把 `set -euo pipefail\r` 解析成非法选项；根因二是 bash 脚本内使用 Windows 风格 `TMPDIR` 重定向路径不可靠                                            | 本地执行 `scripts/format_code.py` 与 `scripts/run_tests.py foundation -- -q`，确认 bootstrap 测试通过                                                                                                                                                                        |
| 109  | 升级 `chatbi-file-ingestion` 为“上传即分析”链路：上传 CSV/XLSX 后先做表头/类型校验，若完整匹配 `sales_order` 或 `customer_profile` 就直接输出结构化业务分析；若不匹配则自动回退到 Pandas 通用画像（shape、dtype、空值、数值摘要、类别分布）；同步更新 `SKILL.md`、上传提示词、Agent prompt 与 `tests/test_file_ingestion_skill.py` 回归用例                                                                                                                               | 当前仅支持读取后分析，不会把上传数据写入 MySQL；Pandas fallback 偏重通用画像，不会伪装成治理后的业务口径；运行前需确保 `.venv` 已安装新增依赖 `pandas`                    | 本地执行 `PYTHONPATH=. .venv/bin/python scripts/format_code.py` 与 `PYTHONPATH=. .venv/bin/python scripts/run_tests.py skills data-sources -- -q`，并在前端上传一份匹配业务表和一份通用表分别验证两条分流                                                                    |
| 110  | 修复上传文件“生成图表”链路在 ReAct 中反复绕圈并误回退到 `chatbi-semantic-query`：`backend/agent/executor.py` 对上传文件的分析/可视化追问自动补 `--include-rows`，确保 `chatbi-file-ingestion` 返回完整行数据；`backend/agent/react_runner.py` 在调用 `chatbi-chart-recommendation` 时自动把上一步工具结果里的 `rows` 与原问题封装成 JSON 传入，避免图表技能只拿到用户原话而缺少数据；补 `tests/test_executor_file_ingestion_args.py` 与 `tests/test_react_runner.py` 回归 | 本次基于代码链路复现与修复，未拿到运行中 Docker 容器的 trace 日志；若后续仍有模型收尾 JSON 非法问题，还需要继续加 LLM finish 阶段的重试/清洗                              | 本地执行 `PYTHONPATH=. .venv/bin/python scripts/format_code.py` 与 `PYTHONPATH=. .venv/bin/python scripts/run_tests.py agent data-sources -- -q`，再在前端对上传文件发送“生成可视化图表”验证不再重复调用 skill 或退回演示库 SQL                                              |
| 112  | 修复上传文件分析在 ReAct 中重复执行 `chatbi-file-ingestion`：`backend/agent/react_runner.py` 新增“同一轮已成功读取上传文件时短路重复调用”的护栏，第二次命中同一 skill 时直接复用当前 Observation 收口；`backend/agent/prompt_builder.py` 补充约束，要求拿到 `file_ingestion` Observation 后优先 `finish` 或转 `chatbi-chart-recommendation`，不要重复读取同一文件；新增 `tests/test_react_runner.py` 回归覆盖重复读取短路 | 当前只解决“同一轮对同一上传文件反复重读”的绕圈问题；`chatbi-decision-advisor` 仍是面向演示业务库销售/留存指标的技能，不能直接拿来解释任意上传的贷款/风控表                     | 本地执行 `PYTHONPATH=. .venv/bin/python scripts/run_tests.py agent -- -q` 与 `PYTHONPATH=. .venv/bin/python -m pytest tests/test_react_runner.py -q`，然后在前端复测上传文件问“逾期贷款分布”确认不再重复调用 `chatbi-file-ingestion`                                      |
| 113  | 增强 `chatbi-file-ingestion` 的后续追问能力：`executor.py` 在上传文件场景仅对“分析/统计/分布/汇总”等后续分析问法透传 `--question`，`inspect_uploaded_table.py` / `_analysis.py` 在 `pandas_fallback` 下按问题匹配字段并直接产出确定性分布统计（如“账户状态统计”）；补 `tests/test_file_ingestion_skill.py` 与 `tests/test_executor_file_ingestion_args.py` 覆盖字段统计与参数透传 | 当前先支持最常见的“字段分布/统计/占比/构成”类问题；更复杂的跨字段聚合、条件过滤和风控建议仍需要单独的上传数据分析/决策技能                                           | 本地执行 `PYTHONPATH=. .venv/bin/python -m pytest tests/test_file_ingestion_skill.py tests/test_executor_file_ingestion_args.py -q` 与 `PYTHONPATH=. .venv/bin/python scripts/run_tests.py agent data-sources -- -q`，再在前端复测“账户状态统计”“贷款状态分布”这类追问 |
| 111  | 收紧 PDF 导出链的顶层导入成本：`backend/report/pdf_chart_png.py` 改为在真正渲染 PNG 时才导入并初始化 `matplotlib`，`backend/report/pdf_report.py` 仅在图表导出分支按需导入图表渲染器，`backend/routes/sessions_route.py` 仅在命中 `/report.pdf` 路由时再导入 `render_session_pdf_bytes`；补 `tests/test_report_pdf.py` 保护 `pdf_report` 与 `sessions_route` 纯导入不再 eager load `matplotlib` | `test_report_pdf.py` 里真正渲染 PDF / PNG 的几条测试仍会偏慢，这是功能真实性能成本；本次主要消掉普通测试 discovery 和非 PDF 路由被连带拖慢的问题                         | 本地执行 `PYTHONPATH=. .venv/bin/python scripts/format_code.py` 与 `PYTHONPATH=. .venv/bin/python scripts/run_tests.py report admin -- -q`，再对比 VSCode Refresh Tests 或 `pytest --collect-only` 的体感速度                                                                 |
| 114  | 将应用/管理/日志表默认收敛回 `chatbi_demo`，并改为沿用原库名前缀：新增 `backend/db_tables.py` 统一 `chatbi_app_*` / `chatbi_admin_*` / `chatbi_logs_*` 常量；`config.py` 让 `CHATBI_APP_DB_NAME` / `CHATBI_ADMIN_DB_NAME` / `CHATBI_LOG_DB_NAME` 默认回落到主库；`database/init.sql` 改为单库建表；新增 `database/migrations/006_merge_feature_tables_with_prefixes.sql` 兼容旧 `chatbi_app` / `chatbi_admin` / `chatbi_logs` 与早期无前缀表的数据迁移；补 `tests/test_config_db_defaults.py` 覆盖默认值 | 代码与初始化脚本已切到单库前缀表，但 `docs/architecture` / `docs/tech-guide` 等历史文档仍保留“四库”表述，后续需要统一刷新；增量迁移脚本未在真实 MySQL 上实测，只做了代码审阅与相关 Python 回归 | 本地执行 `PYTHONPATH=. .venv/bin/python scripts/format_code.py`、`PYTHONPATH=. .venv/bin/python scripts/run_tests.py admin -- -q`、`PYTHONPATH=. .venv/bin/python scripts/run_tests.py data-sources -- -q`；如需把现有库搬到新表名，再执行 `database/migrations/006_merge_feature_tables_with_prefixes.sql` |
| 115  | 复核日志库本地连接分离：确认后端已有 `CHATBI_LOG_DB_*` 专用连接；将 `.env.dev` / `.env.example` 的日志库改为 `chatbi_local_logs` 且本地端口 `33067`；`docker-compose.dev.yml` 的 backend 日志连接改为 `host.docker.internal:33067`，避免环境覆盖回 `demo-mysql:3306`；Skill 侧 trace 表名对齐 `chatbi_logs_trace_log`；补 `tests/test_trace.py` 覆盖独立端口与表名 | `scripts/bootstrap_dev.sh` 和 `scripts/format_code.py` 当前会在前端 ESLint 阶段因 `frontend/node_modules/eslint/package.json` 无效而失败；未实际连接 `127.0.0.1:33067` 验证 MySQL 服务是否已启动 | 修复/重装前端 `node_modules` 后重跑完整格式化；启动本地日志 MySQL 监听 `33067` 后，用固定 trace-id 查询 `chatbi_local_logs.chatbi_logs_trace_log` |
| 116  | 精简同库部署 env：`.env.example` / `.env.dev` 删除重复的 `CHATBI_APP_DB_*` 与 `CHATBI_ADMIN_DB_*`，只保留主业务库 `CHATBI_DB_*` 和独立日志库 `CHATBI_LOG_DB_*`；dev/prod compose 也删除 APP/ADMIN 覆盖，让应用表和管理表自然回落到业务库；README 标注 APP/ADMIN 变量仅在主动拆库时可选 | `CHATBI_APP_DB_*` / `CHATBI_ADMIN_DB_*` 仍保留在代码中作为兼容扩展点；`scripts/format_code.py` 仍因本地前端 eslint 包配置损坏在 ESLint 阶段失败 | 重装前端依赖后重跑完整格式化；如未来需要再次拆出应用库/管理库，只需显式补回对应 env |
| 117  | 验证当前 `.env.dev` 指向的独立日志库 `127.0.0.1:33067/chatbi_local_logs`：`mysql` 与 `mysqladmin ping` 均返回无法连接；代码侧预期日志表为 `chatbi_local_logs.chatbi_logs_trace_log`，迁移脚本 `007_move_trace_logs_to_local_db.sql` 可用于初始化 | 当前宿主机 `33067` 没有可用 MySQL 监听；`3308` dev 业务库也无法连接；`docker ps` 因本机 Docker socket 权限无法读取；`bootstrap_dev.sh` 仍卡在前端 `eslint/package.json` 无效 | 先启动或修复本地 MySQL，使 `33067` 暴露可连；然后执行 `database/migrations/007_move_trace_logs_to_local_db.sql` 初始化日志库，再用固定 trace-id 插入并查询 `chatbi_logs_trace_log` |
| 118  | 审计日志库独立化与同库前缀表改动是否落地：代码配置、`database/init.sql`、迁移脚本、compose、README、架构文档、用户/技术指南均已对齐；生产 compose 补 `CHATBI_LOG_DB_NAME=chatbi_local_logs`，避免无 `.env` 时日志回落到 `chatbi_demo`；历史 `current-sprint` 记录保留旧表述作为流水账 | `scripts/format_code.py` 的 Python ruff/black 阶段通过，但前端 ESLint 仍因 `frontend/node_modules/eslint/package.json` 无效失败；真实 `33067` MySQL 仍未启动，未做 live 写入验证 | 重装/修复前端依赖后重跑完整格式化；启动 `33067` 日志 MySQL 后执行 `007_move_trace_logs_to_local_db.sql` 并用固定 trace-id 做真实写入查询 |
