...
| 轮次 | 完成内容                                                                                                                                                 | 验证                                                                                                                                                                                      |
| ---- | -------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 222  | 基于当前仓库状态重写主 README：同步下线功能、真实路由、启动方式、目录结构、Skill 概览与文档导航，收敛旧文案中已过时的仪表盘 / PDF / 数据源页面描述    | 文档自查：核对 `frontend/src/App.tsx`、`frontend/src/components/AppLayout.tsx`、`backend/main.py`、`docker-compose*.yml`、`.env.example`、`docs/architecture/README.md`               |
| 221  | 删除会话 PDF 导出能力：移除聊天页按钮、前端下载 API、后端 `report.pdf` 路由、PDF 报告模块、测试套件与相关依赖文档                                     | `uv lock`；`PYTHONPATH=. .venv/bin/python scripts/format_code.py backend/routes/sessions_route.py frontend/src/pages/ChatPage.tsx frontend/src/api/client.ts scripts/run_tests.py`；`PYTHONPATH=. .venv/bin/python scripts/run_tests.py foundation -- -q`；`cd frontend && npm run lint`；`cd frontend && npm run build` |
| 220  | 收起数据源管理 UI：移除侧栏入口、独立页面、聊天页手填连接 ID 与前端连接管理 API；`/data-sources` 现在重定向回对话首页，后端连接管理接口暂时保留备用    | `PYTHONPATH=. .venv/bin/python scripts/format_code.py frontend/src/App.tsx frontend/src/components/AppLayout.tsx frontend/src/pages/ChatPage.tsx frontend/src/hooks/useChat.ts frontend/src/api/client.ts frontend/src/types/admin.ts`；`cd frontend && npm run lint`；`cd frontend && npm run build` |
