...
实现：[`backend/memory_service.py`](../backend/memory_service.py) + [`backend/memory_repo.py`](../backend/memory_repo.py)，表 [`user_memory`](../database/init.sql)（库 `chatbi_app`）。
...
| `chatbi_app` | **用户**（`app_user`）、**会话**（`chat_session`）、**消息**（`chat_message`）、**记忆**（`user_memory`） |
| `chatbi_admin` | **Skill 开关**（`skill_registry`）、**数据源连接**（`app_db_connection`）、**LLM 覆盖配置**（`llm_settings`） |
| `chatbi_logs` | **链路日志**（`chatbi_trace_log`，trace-id 串联上传、chat、Skill 等） |
...
所有 Skill 均在 `skills/<目录名>/SKILL.md` 注册；**目录名**即 **`skill` 字段使用的 slug**。启用状态由 `chatbi_admin.skill_registry` 过滤（[`scan_skills_enabled`](../backend/agent/prompt_builder.py)）。
...
| `chatbi-metric-explainer` | 指标口径解释类 | 专线 registry 中分配给 marketing/analysis 等 |
...
| ReAct / Legacy 验收清单 | [`docs/design/agent-runtime-acceptance.md`](design/agent-runtime-acceptance.md) |
