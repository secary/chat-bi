...
METRICS: List[Tuple[MetricDef, Tuple[str, ...]]] = [
    (
        MetricDef("corporate_deposit_balance", "对公存款余额", "deposit", "sum", "CNY"),
        ("对公存款余额", "公司存款余额", "企业存款余额", "对公存款规模"),
    ),
    (
        MetricDef("retail_deposit_balance", "个人存款余额", "deposit", "sum", "CNY"),
        ("个人存款余额", "零售存款余额", "储蓄存款余额", "个人存款规模"),
    ),
    (
        MetricDef(
            "deposit_balance", "存款余额", "deposit", "sum", "CNY", requires_business_line=True
        ),
        ("存款余额", "存款规模", "存款额", "存款总额", "时点存款"),
    ),
    (
        MetricDef("corporate_loan_balance", "对公贷款余额", "loan", "sum", "CNY"),
        ("对公贷款余额", "企业贷款余额", "对公信贷余额"),
    ),
    (
        MetricDef("retail_loan_balance", "个人贷款余额", "loan", "sum", "CNY"),
        ("个人贷款余额", "零售贷款余额", "个贷余额"),
    ),
    (
        MetricDef("inclusive_loan_balance", "普惠贷款余额", "loan", "sum", "CNY"),
        ("普惠贷款余额", "小微贷款余额", "普惠金融贷款余额"),
    ),
    (
        MetricDef("loan_balance", "贷款余额", "loan", "sum", "CNY", requires_business_line=True),
        ("贷款余额", "贷款规模", "信贷余额"),
    ),
    (
        MetricDef("corporate_customer_count", "对公客户数", "customer", "count_distinct", None),
        ("对公客户数", "企业客户数", "公司客户数"),
    ),
    (
        MetricDef("retail_customer_count", "个人客户数", "customer", "count_distinct", None),
        ("个人客户数", "零售客户数", "储蓄客户数"),
    ),
    (
        MetricDef(
    ...
    (
        MetricDef(
            "mobile_active_user_count", "手机银行活跃用户数", "channel", "count_distinct", None
        ),
        ("手机银行活跃用户数", "手机银行活客", "掌银活跃用户", "月活用户", "mau"),
    ),
    (
        MetricDef("transaction_amount", "交易金额", "transaction", "sum", "CNY"),
        ("交易金额", "交易额", "流水金额", "支付金额"),
    ),
    (
        MetricDef("transaction_count", "交易笔数", "transaction", "count", None),
        ("交易笔数", "笔数", "交易次数"),
    ),
    (
        MetricDef("aum_balance", "AUM余额", "wealth", "sum", "CNY"),
        ("aum余额", "aum", "金融资产余额", "客户资产余额"),
    ),
    (
        MetricDef("fee_income", "中间业务收入", "income", "sum", "CNY"),
        ("中间业务收入", "手续费收入", "中收", "非息收入"),
    ),
    (
        MetricDef("interest_income", "利息收入", "income", "sum", "CNY"),
        ("利息收入", "息收", "贷款利息收入"),
    ),
    (
        MetricDef(
            "npl_ratio", "不良贷款率", "risk", "ratio", None, formula="不良贷款余额 / 贷款余额"
        ),
        ("不良贷款率", "不良率", "贷款不良率"),
    ),
]
...
def parse_question(question: str, today: Optional[date] = None) -> Dict[str, Any]:
    ...
    query_intent = {
        ...
        "sql_readiness": {
            ...
            "notes": (
                [] if not missing_slots else ["仍存在关键语义槽位缺失，暂不建议直接生成 SQL。"]
            ),
        },
    }
    return query_intent
...
def render_summary(query_intent: Dict[str, Any]) -> str:
    ...
    question = (
        query_intent["clarification_questions"][0]
    ...
    for metric, words in METRICS:
        for word in words:
            if word.lower() in normalized:
                score = len(word) + (
                    10
                    if business_line != "unknown"
                    and metric.metric_id.startswith(business_line.split("_")[0])
                    else 0
                )
...
def _parse_time(normalized: str, today: date) -> Tuple[Dict[str, Any], List[str]]:
    ...
    if month_range:
        ...
        return (
            _time_payload(start, end, "month", "余额" in normalized or "规模" in normalized),
            assumptions,
        )
    ...
    if "去年" in normalized:
        return (
            _time_payload(date(today.year - 1, 1, 1), date(today.year - 1, 12, 31), "year", False),
            assumptions,
        )
    ...
    return (
        _time_payload(
            prev_month_start, prev_month_end, "month", "余额" in normalized or "规模" in normalized
        ),
        assumptions,
    )
...
def _time_payload(
    start: date, end: date, grain: Optional[str], point_in_time: bool
) -> Dict[str, Any]:
    return {
        "time_range": {
...
def _build_comparison(normalized: str) -> Dict[str, Any]:
    if "同比" in normalized:
        return {
            "type": "year_over_year",
            "base_range": "prior_year_same_period",
            "calculation": "growth_rate",
        }
    if "环比" in normalized:
        return {
            "type": "period_over_period",
            "base_range": "previous_comparable_period",
            "calculation": "growth_rate",
        }
    ...
    if "较上月末" in normalized:
        return {
            "type": "vs_previous_month_end",
            "base_range": "previous_month_end",
            "calculation": "delta",
        }
...
def _collect_gaps(
    ...
    if metric is None:
        ...
        ambiguities.append(
            {
                "slot": "business_line",
                "candidates": ["corporate", "retail", "inclusive_finance", "whole_bank"],
            }
        )
...
def _schema_hints(
    metric: Optional[MetricDef], dimensions: List[Dict[str, str]]
) -> List[Dict[str, Any]]:
    hints: List[Dict[str, Any]] = []
    if metric:
        hints.append(
            {
                "canonical_id": metric.metric_id,
                "type": "metric",
                "candidate_tables": [],
                "candidate_columns": [],
                "join_keys": [],
    ...
    for item in dimensions[:3]:
        hints.append(
            {
                "canonical_id": item["dimension_id"],
                "type": "dimension",
                "candidate_tables": [],
                "candidate_columns": [],
                "join_keys": [],
